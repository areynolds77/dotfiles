...
source /dev/stdin <<< "$(curl -fsSL https://raw.githubusercontent.com/areynolds77/dotfiles/mac/script/prompt)"
...
echo ''
read -p "Do you want to use areynolds77's dotfiles? y/n" -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Yy]$ ]]
then
    echo ''
	echo "Now pulling down  dotfiles..."
	git clone https://github.com/areynolds77/dotfiles.git ~/.dotfiles
	echo ''
    ...
    if [[ $? -eq 0 ]]
    then
        echo "Successfully configured your environment with areynolds77's macOS dotfiles..."
    else
        echo "areynolds77's macOS dotfiles were not applied successfully..." >&2
fi
else 
	echo ''
    echo "You chose not to apply areynolds77's macOS dotfiles. You will need to configure your environment manually..."
	echo ''
